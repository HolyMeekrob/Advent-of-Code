defmodule DayTwo do
	...
	def part_two(rows) do
		rows
		|> Enum.map(&even_divide/1)
		|> List.foldl(0, &(+/2))
	end

	defp checksum(row) do
		Enum.max(row) - Enum.min(row)
	end

	defp even_divide(row) do
		row
		|> List.foldl({row, {}}, &find_divisible_pair/2)
		|> divide_pair
	end

	defp find_divisible_pair(num, {row, {}}) do
		evenly_divides_another = and2(&evenly_divides/2, &(!=/2))
		match = Enum.find(row, &(evenly_divides_another.(num, &1)))
	...
	defp find_divisible_pair(_, {row, pair}) do
		{row, pair}
	end

	defp divide_pair({_, {x, y}}) do
		div(x, y)
	end

	defp evenly_divides(x, y) do
		rem(x, y) == 0
	end

	defp and2(f, g) do
		fn(x, y) -> f.(x, y) && g.(x, y) end
	end
end
...
IO.puts("Part two: " <> Integer.to_string(DayTwo.part_two(input)))
