...
namespace Advent._2020
{
	class Day02 : IDay
	{
		...
		private static PasswordData GetPasswordData(string input)
		{
			...
			{
				SmallNum = Convert.ToInt32(groups[1].Value),
				LargeNum = Convert.ToInt32(groups[2].Value),
		...
		private static string GetPartOne(List<PasswordData> input) =>
			input
				.Count(IsValidPartOne)
				.ToString();

		private static string GetPartTwo(List<PasswordData> input) =>
			input
				.Count(IsValidPartTwo)
				.ToString();

		public (string, string) GetResult()
		{
			var parsedInput = Input.Select(GetPasswordData).ToList();
			return (GetPartOne(parsedInput), GetPartTwo(parsedInput));
		}

		private record PasswordData
		{
			public int SmallNum { get; init; }
			public int LargeNum { get; init; }
			public char RequiredCharacter { get; init; }
