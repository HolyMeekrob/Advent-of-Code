...
namespace Advent._2020
{
	public class Day01 : IDay
	{
		...
		private static string GetPartOne()
		{
			...
			{
				var matchIndex = Array.BinarySearch(sortedInput, 2020 - sortedInput[i]);
				if (matchIndex > -1)
				{
					return (sortedInput[i] * sortedInput[matchIndex]).ToString();
				}
			}
		...
		private static string GetPartTwo()
		{
			...
			for (int i = 0; i < sortedInput.Length; ++i)
			{
				...
				for (int j = i + 1; j < sortedInput.Length; ++j)
				{
					...
					var matchIndex = Array.BinarySearch(sortedInput, 2020 - ab);
					if (matchIndex > -1)
					{
						return (a * b * sortedInput[matchIndex]).ToString();
					}
				}
			}
